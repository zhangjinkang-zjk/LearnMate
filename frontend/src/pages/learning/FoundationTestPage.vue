...
async function loadPage() {
  ...
  nodeError.value = ''
  try {
    const [current, paths, overview] = await Promise.all([
      fundamentalsApi.getCurrentPath(route.query.pathId),
      fundamentalsApi.listPaths(),
      learningApi.getOverview().catch(() => null),
    ])
    learningPath.value = current
    pathCatalog.value = Array.isArray(paths) ? paths : (paths?.paths || [])
    if (overview) applyInsights(overview)
