    ...
    <div v-else class="overview-layout">
      <main class="overview-main">
        <section class="surface surface-pad goal-panel">
          <div class="section-heading"><div><p class="eyebrow">当前目标</p><h2>{{ profile.goal }}</h2></div><span class="goal-progress">{{ path.progress }}%</span></div>
          ...
          <div class="goal-meta"><span>已完成 {{ path.completedNodes }} / {{ path.totalNodes }} 个学习节点</span><span v-if="path.currentNode">当前节点：{{ path.currentNode }}</span></div>
        </section>
        ...
        <section class="surface surface-pad">
          ...
          <div v-else class="empty-state">还没有生成学习路径，完成学习定向后这里会自动出现相关科目。</div>
        </section>
        ...
        <section class="surface surface-pad diagnosis-panel">
          ...
          <div class="diagnosis-summary"><div class="diagnosis-score"><strong>{{ diagnosis.score }}%</strong><span class="muted">综合掌握度</span></div><div class="diagnosis-stats"><span><strong>{{ stats.examAnswered }}</strong> 道题已作答</span><span><strong>{{ formatDuration(stats.studySeconds) }}</strong> 累计学习</span></div></div>
          ...
          <p v-else class="muted diagnosis-note">当前还没有足够的练习记录，完成一个学习节点后会更新诊断。</p>
        </section>
      </main>
      ...
      <aside class="overview-aside">
        ...
        <section class="surface surface-pad next-node-panel"><p class="eyebrow">路径进度</p><div class="next-node-heading"><h2>接下来会学</h2><span class="muted">{{ path.stage }}</span></div><p class="muted">{{ path.currentNode || '完成学习定向后，系统会生成第一个学习节点。' }}</p><RouterLink class="text-link" to="/learning/navigation">查看完整路径 →</RouterLink></section>
      </aside>
    </div>
  </div>
</template>
...
const path = reactive({ progress: 0, stage: '尚未开始', currentNode: '', completedNodes: 0, totalNodes: 0, nodes: [], diagnosis: null, nextAction: null })
...
const diagnosis = computed(() => {
  ...
  return { score, weakPoints: unique.map((item) => ({ ...item, accuracy: Math.round(Number(item.accuracy || 0) * 100) })) }
})

const recommendation = computed(() => {
  const current = path.currentNode || (path.progress >= 100 ? '复盘已完成内容' : '当前学习节点')
