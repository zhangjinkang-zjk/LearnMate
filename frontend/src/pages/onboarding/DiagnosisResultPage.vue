  ...
  <main class="result-page">
    ...
    <div class="result-grid">
      ...
      <section class="result-card result-list">
        ...
        <p v-if="isPreparing" class="result-status" role="status">正在分析你的画像与诊断结果，为你生成学习路径…</p>
        <p v-else-if="prepareError" class="result-status result-status--error" role="alert">{{ prepareError }}</p>
        <div class="result-actions">
          <RouterLink class="button button--quiet" to="/onboarding/diagnosis">重新诊断</RouterLink>
          <button class="button button--primary" type="button" :disabled="isPreparing" @click="enterOverview">进入学习概览</button>
        </div>
      </section>
    </div>
  </main>
...
// 路径由诊断收尾在后台生成（后端 _generate_paths_after_diagnosis）。这里等它就绪再放行，
// 否则用户点进学习概览只会看到空页面。超时也放行，只是提示一下，不把人锁在结果页。
const checkReady = async () => {
  try {
    const current = unwrap(await learningApi.getCurrentPath())
    if (current?.path_id) {
      isPreparing.value = false
      stopPolling()
      return
    }
  } catch { /* 轮询期间的偶发失败交给下一次 */ }
  if (Date.now() > pollDeadline) {
    isPreparing.value = false
    prepareError.value = '学习路径还在生成，进入学习概览后可能需要稍等才能看到内容。'
    stopPolling()
  }
}

const enterOverview = () => router.push('/learning/overview')

onMounted(() => {
  pollDeadline = Date.now() + 180000
  void checkReady()
  pollTimer = setInterval(() => void checkReady(), 3000)
})
